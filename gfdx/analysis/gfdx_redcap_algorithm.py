...
Original file is located at
    https://colab.research.google.com/drive/1hlm-T5YeqDWizUBKFvV1GqFCvoM4IoN_
...
fields_of_interest = ['country_code', 'food_intake', 'import_pc', 'coverage_fv', 'industrially_processed_pc', 
                      ...
                      'fortification_standard', 'ext_mon_protocol', 'imp_mon_protocol', 'mandatory_fortification', 'voluntary_fortification']
...
oil = 'oil_arm_1'
salt = 'salt_arm_1'
maize = 'maize_flour_arm_1'
wheat = 'wheat_flour_arm_1'
rice = 'rice_arm_1'
...
# Step 1a - categorize avaiablilty/intake
# Create formula comment
def identify_availability_comment(row):
    return_val = ''
    if row.redcap_event_name == maize:
        if row.food_intake < 75:
            return_val = 'Low intake/availability: Maize flour is not consumed in adequate amounts to fortify alone for some nutrients. Consider fortifying a food with greater intake or another food in addition to maize flour.'
        elif row.food_intake < 150:
            return_val = 'Moderate intake/availability'
        elif row.food_intake < 300:
            return_val = 'Moderately high intake/availability'
            ...
            return_val = 'Unknown intake/availability'
    elif row.redcap_event_name == wheat:
        if row.food_intake < 75:
            return_val = 'Low intake/availability: Wheat flour is not consumed in adequate amounts to fortify alone for some nutrients. Consider fortifying a food with greater intake or another food in addition to wheat flour.'
        elif row.food_intake < 150:
            return_val = 'Moderate intake/availability'
        elif row.food_intake < 300:
            return_val = 'Moderately high intake/availability'
            ...
            return_val = 'Unknown intake/availability'
    elif row.redcap_event_name == rice:
        if row.food_intake < 75:
            return_val = 'Low intake/availability: Rice is not consumed in adequate amounts to fortify alone for some nutrients. Consider fortifying a food with greater intake or another food in addition to rice.'
        elif row.food_intake < 150:
            return_val = 'Moderate intake/availability'
        elif row.food_intake < 300:
            return_val = 'Moderately high intake/availability'
        ...
        if row.food_intake < 20:
            return_val = 'Low intake/availability: Oil is not consumed in adequate amounts to fortify alone for some nutrients. Consider fortifying a food with greater intake or another food in addition to oil.'
        elif row.food_intake < 40:
            return_val = 'Moderate intake/availability'
        elif row.food_intake < 60:
            return_val = 'Moderately high intake/availability'
...
new_df['food_avail_comment'] = new_df.apply(lambda row: identify_availability_comment(row), axis=1)
...
def assess_reach_comment(x, y, z):
    ...
    if x == 'Low intake/availability' and y == 'Low population coverage':
        ...
        return_val = f'Benefit from fortifying {z} appears low but more data on population coverage of {z} are needed to understand whether fortification will benefit the population.'
    elif x == 'Moderate intake/availability' and y == 'Low population coverage':
        return_val = f'Benefit from fortifying {z} is moderate.'
    elif x == 'Moderate intake/availability' and y == 'Moderate population coverage':
        return_val = f'Benefit from fortifying {z} is moderate.'
        ...
        return_val = f'Benefit from fortifying {z} appears moderate but more data on population coverage of {z} are needed to understand whether fortification will benefit the population.'
    elif x == 'Moderately high intake/availability' and y == 'Low population coverage':
        return_val = f'Benefit from fortifying {z} is moderate.'
    elif x == 'Moderately high intake/availability' and y == 'Moderate population coverage':
        return_val = f'Benefit from fortifying {z} is moderate.'
        ...
        return_val = f'Benefit from fortifying {z} appears high but more data on population coverage of {z} are needed to understand whether fortification will benefit the population.'
    elif x == 'Unknown intake/availability' and y == 'Unknown population coverage':
        return_val = f'More data are needed to better understand consumption patterns and population coverage.'
    elif x == 'Unknown intake/availability' and y == 'Low population coverage':
        return_val = f'Benefit from fortifying {z} appears low but more data on dietary intake amounts of {z} are needed to understand whether fortification will benefit the population.'
    elif x == 'Unknown intake/availability' and y == 'Moderate population coverage':
        return_val = f'Benefit from fortifying {z} appears moderate but more data on dietary intake amounts of {z} are needed to understand whether fortification will benefit the population.'
    elif x == 'Unknown intake/availability' and y == 'High population coverage':
        return_val = f'Benefit from fortifying {z} appears high but more data on dietary intake amounts of {z} are needed to understand whether fortification will benefit the population.'
...
def assess_industry_comment(df, z):
    ...
    if salt_event == 'salt_arm_1':
        ...
        return f'A low proportion of {z} is industrially processed; fortifying {z} may not be cost-effective and sustainable.'
    elif industrial_var < 75:
...
for country in new_df.country_code.unique():
    for food in new_df.redcap_event_name.unique():
            ...
            food_avail_comment = temp[temp.redcap_repeat_instrument == 'intake'].food_avail_comment.iloc[0]
            ...
            food_availabilities_comment.append(food_avail_comment)
...
final['intake_cat_comment'] = food_availabilities_comment
...
for country in filt_df.country_code.unique():
    for food in filt_df.redcap_event_name.unique():
        ...
        man_stat.append(small.mandatory_fortification.iloc[0])   
        vol_stat.append(small.voluntary_fortification.iloc[0])              
...
final['man_status'] = man_stat
final['vol_status'] = vol_stat
...
def status_comparison(row):
    ...
    elif row.man_status == 2 and row.vol_status == 2 and row.overall_recommendation == 'Good':
        return f'Fortification of {row.food_lower} is not mandatory; consider introducing mandatory fortification.'
    elif row.man_status == 2 and row.vol_status == 2 and row.overall_recommendation == 'Moderate':
        return f'Fortification of {row.food_lower} is not mandatory; consider introducing mandatory fortification.'
    elif row.man_status == 2 and row.vol_status == 2 and row.overall_recommendation == 'Poor':
        return f'Fortification of {row.food_lower} is not mandatory; consider exploring mandatory fortification of other foods that are a good fortification opportunity.'
    elif row.man_status == 2 and row.vol_status == 2 and row.overall_recommendation == 'More data needed':
        return f'Fortification of {row.food_lower} is not mandatory and there is not enough data to determine whether {row.food_lower} is a good opportunity; consider exploring mandatory fortification of other foods that are a good fortification opportunity.'  
    elif row.man_status == 3 and row.overall_recommendation == 'Good':
    ...
    elif row.man_status == 3 and row.overall_recommendation == 'Moderate':
    ...
    elif row.man_status == 3 and row.overall_recommendation == 'Poor':
    ...
    elif row.man_status == 3 and row.overall_recommendation == 'More data needed':
...
def standard_comparison(row):
    if row.status_food == 1 and row.fortification_standard == 1:
        return 'No Assessment'
    elif row.status_food == 1 and row.fortification_standard == 2:
        return f'Consider developing standards for {row.food_lower} to provide guidance to industry on the quality and nutrient specifications their product must meet.'
    ...
    elif row.status_food == 2 and row.fortification_standard == 1:
        return 'No Assessment'
...
# Step 4 - Comparing legislation status with Presence of external monitoring protocols
# Create formula

def external_comparison(row):
    if row.status_food == 1 and row.ext_mon_protocol == 1:
        return 'External monitoring protocols exist, which should indicate clarity on roles and responsibilities of stakeholders, monitoring frequency and procedures, and reporting and sharing of data with industry. Consider reviewing these protocols and making improvements to them if needed.'
    elif row.status_food == 1 and row.ext_mon_protocol == 2:
        return 'Consider developing external monitoring protocols for this food to define roles and responsibilities of stakeholders, monitoring frequency and procedures, and reporting and sharing of data with industry.'
    elif row.status_food == 1 and row.ext_mon_protocol == 3:
        return f'GFDx does not have the external monitoring protocol for {row.food_lower}.'
    else:
...
def uses_pf(row):
    if row.status_food == 1 and row.processed_food == 1:
    ...
    elif row.status_food == 1 and row.processed_food == 0:
...
final['external_comparison'] = final.apply(lambda row: external_comparison(row), axis=1)
...
final.drop(['status_food','man_status', 'vol_status','fortification_standard', 
